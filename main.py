...
def main(args):
    ...
    if args.im_path1 != '' and args.im_path2 != '':
        ii2s = Embedding(args)
        #
        ...
        imagePaths = [im_path1, im_path2, im_path3]

        for x in imagePaths:
            image_name = os.path.splitext(os.path.basename(x))[0]
            if (image_name != ''):
                image_weight_path = os.path.join(args.output_dir, 'W+/{}.npy'.format(image_name))
                image_fs_path = os.path.join(args.output_dir, 'FS/{}.npz'.format(image_name))
                if not os.path.exists(image_weight_path):
                    ii2s.invert_images_in_W(x)
                if not os.path.exists(image_fs_path):
                    ii2s.invert_images_in_FS(x)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--im_path1', type=str, default='', help='Identity image')
    parser.add_argument('--im_path2', type=str, default='', help='Structure image')
    parser.add_argument('--im_path3', type=str, default='', help='Appearance image')
    ...
    parser.add_argument('--W_steps', type=int, default=50, help='Number of W space optimization steps') # default 1100
    parser.add_argument('--FS_steps', type=int, default=50, help='Number of W space optimization steps') # default 250
    ...
    parser.add_argument('--align_steps1', type=int, default=10, help='') # default 140
    parser.add_argument('--align_steps2', type=int, default=10, help='') # default 100
    parser.add_argument('--mask_steps1', type=int, default=10, help='') # default 80
    parser.add_argument('--mask_steps2', type=int, default=10, help='') # default 80


    # Blend loss options
    parser.add_argument('--face_lambda', type=float, default=1.0, help='')
    parser.add_argument('--hair_lambda', type=str, default=1.0, help='')
    parser.add_argument('--blend_steps', type=int, default=50, help='') # default 400
