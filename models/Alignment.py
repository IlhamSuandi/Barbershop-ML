...
class Alignment(nn.Module):
    ...
    def create_target_segmentation_mask(self, img_path1, img_path2, sign, save_intermediate=True):
        ...
        vis_path = os.path.join(self.opts.output_dir, 'vis_mask_{}_{}_{}.png'.format(im_name_1, im_name_2, sign))

        if not os.path.exists(vis_path):
            print ("visual image is not exist")
            print (vis_path)

            optimizer_align, latent_align = self.setup_align_optimizer()
            latent_end = latent_align[:, 6:, :].clone().detach()

            pbar = tqdm(range(self.opts.mask_steps1), desc='Create Target Mask Step1', leave=False)
            for step in pbar:
            ...
            pbar = tqdm(range(self.opts.mask_steps2), desc='Create Target Mask Step2', leave=False)
            for step in pbar:
                optimizer_align.zero_grad()
                latent_in = torch.cat([latent_align[:, :6, :], latent_end], dim=1)
                down_seg, _ = self.create_down_seg(latent_in)
